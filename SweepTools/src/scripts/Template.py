...
from System.Windows.Forms import (
    ...
    Form, Label, ComboBox, CheckBox, NumericUpDown,
    MessageBox, Application, Panel, TableLayoutPanel,
    FlowLayoutPanel, FlowDirection, ColumnStyle, RowStyle,
    SizeType, RadioButton
)
from System.Drawing import Color, Size, Point as gPoint, SizeF, Font, FontStyle, SystemFonts
...
def get_professional_colors():
    return {
        ...
        'panel': Color.FromArgb(245, 245, 245),
        ...
        'button_hover': Color.FromArgb(229, 241, 255)
    }
def clear_alibre_selection(root_or_session):
    """Clear selection per API: pass NULL (None) or an empty IObjectCollector."""
    try:
        sess = getattr(root_or_session, 'TopmostSession', root_or_session)
        try:
            sess.Select(None)  # official NULL path
        except:
            try:
                empty = sess.Root.NewObjectCollector()
            except:
                empty = root.NewObjectCollector() if root is not None else None
            if empty is not None:
                sess.Select(empty)
    except Exception as ex:
        print("Selection clear skipped:", ex)#<-replace
def clear_selection_now_and_soon(root_or_session, host_form=None, delay_ms=25):
    """Clear now and again on next UI tick to beat auto-selection after regen/create."""
    clear_alibre_selection(root_or_session)
    try:
        from System.Windows.Forms import Timer
        t = Timer()
        if host_form is not None:
            try:
                host_form.Disposed += (lambda *_: (t.Stop(), t.Dispose()))
            except:
                pass
        t.Interval = max(1, int(delay_ms))
        def _tick(sender, e):
            try:
                clear_alibre_selection(root_or_session)
            finally:
        ...
        print("Deferred clear skipped:", ex)#<-replace

#////////////////
# SelectionListBox class and Filters class by NateLiquidGravity and sourced 7/23/2025.
# https://liquidgravity.us/store_test/index.php/2025/07/23/752/
#////////////////
class Filters:
    def __init__(self, verbose = 0):
        try:
            import AlibreScript
            root_loc = AlibreScript.API.Global.Root
            self.top_sess = root_loc.TopmostSession
            ...
            show_error('Failed to capture original selection filter: %s' % ex, include_trace=True)
            self.top_sess = None
            self.OriginalFilter = None
    def AllSketches(self, verbose = 0):
        try:
            ...
            try:
                NewSolid = NewFilter.Solid; NewFilter.Solid = NewSolid.AD_NONE
                NewSurface = NewFilter.Surface; NewFilter.Surface = NewSurface.AD_NONE
            except:
                pass
        except Exception as ex:
            show_error('Failed to set sketch selection filter: %s' % ex, include_trace=True)
    def Reset(self, verbose = 0):
        try:
            if self.top_sess is None or self.OriginalFilter is None:
                return
            NewFilter = self.top_sess.SelectionFilter
            ...
            print('Failed to reset selection filter: %s' % ex)#<-replace
...
class SelectionListBox(ListBox):
    ...
    def onEnter_Selection(self, sender, e):
        sender.my_filter.AllSketches()
        colors = get_professional_colors()
        sender.BackColor = colors['accent_light']
        clear_alibre_selection(sender.Root)   # start with empty selection
    ...
    def onLeave_Selection(self, sender, e):
        try:
            ...
            clear_alibre_selection(sender.Root)
    ...
    def onSelectedValueChanged(self, sender, e):
        try:
            ...
            print('onSelectedValueChanged error: %s' % ex)#<-replace
    @safe_try
    def TimerEventProcessor(self, sender, e):
        ...
        try:
            self.myTimer.Stop()
            # --- Defensive checks: SelectedObjects can be null during regen
            if self.top_sess is None:
                return
            ...
            for a in range(0, count):
                ...
                if tgt is not None and 'SKETCH' in tname.upper():
                    ...
                    if self.Items.Count == 0 or sketch_name != self.Items[0]:
                        ...
                        self.SelectedObject = tgt
                    try:
                        self.HoldTimer = 1
                        clear_alibre_selection(self.Root)
                    finally:
                        self.HoldTimer = 0
...
def create_professional_button(text, is_primary=False):
    ...
    if is_primary:
        btn.BackColor = colors['accent']; btn.ForeColor = Color.White
        btn.FlatAppearance.BorderColor = colors['accent']
    else:
        btn.BackColor = colors['button_bg']; btn.ForeColor = colors['text']
...
def create_professional_label(text, is_header=False):
    ...
    return lbl
def create_professional_combobox():
    colors = get_professional_colors()
    cmb = ComboBox()
    cmb.FlatStyle = FlatStyle.Flat
    cmb.BackColor = Color.White
    cmb.ForeColor = colors['text']
    cmb.Font = Font(SystemFonts.DefaultFont.FontFamily, scale_size(9))
    cmb.Margin = Padding(2)
    return cmb
...
def show_form():
    ...
    main_panel = Panel(); main_panel.Dock = DockStyle.Fill
    main_panel.BackColor = colors['background']; main_panel.Padding = Padding(8)
    main_panel.AutoSize = True; main_panel.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink
    form.Controls.Add(main_panel)
    table = TableLayoutPanel(); table.ColumnCount = 1; table.RowCount = 0
    table.Dock = DockStyle.Fill; table.AutoSize = True
    table.AutoSizeMode = System.Windows.Forms.AutoSizeMode.GrowAndShrink
    table.Padding = Padding(0); table.Margin = Padding(0)
    ...
    def add_control_row(ctrl, extra_margin_bottom=None, fixed_height=None):
        ...
        ctrl.Margin = Padding(0, 0, 0, mb); ctrl.Dock = DockStyle.Fill
    ...
    sel_sketch = SelectionListBox(); sel_sketch.Tag = "sketch"; sel_sketch.IntegralHeight = False; sel_sketch.Height = 90
    add_control_row(sel_sketch, fixed_height=90)
    chk_3d = create_professional_checkbox("Use 3D Path"); add_control_row(chk_3d, extra_margin_bottom=section_spacing)
    ...
    rb_mm.Checked = True
    units_flow.Controls.Add(rb_mm); units_flow.Controls.Add(rb_in); units_flow.Controls.Add(rb_cm)
    ...
    prof_flow.Controls.Add(rb_circle); prof_flow.Controls.Add(rb_rect)
    ...
    num_size.Value = 50; num_size.DecimalPlaces = 2; num_size.Maximum = 10000; num_size.Minimum = 0.01; num_size.Height = 28
    ...
    chk_hollow = create_professional_checkbox("Create hollow profile"); add_control_row(chk_hollow)
    lbl_thick = create_professional_label("Wall Thickness:"); add_control_row(lbl_thick)
    num_thick = create_professional_numericupdown()
    num_thick.Value = 2; num_thick.DecimalPlaces = 2; num_thick.Maximum = 1000; num_thick.Minimum = 0.0
    num_thick.Height = 28; num_thick.Enabled = False
    ...
    def current_unit_text():
        if rb_mm.Checked: return "Millimeters"
        if rb_in.Checked: return "Inches"
        if rb_cm.Checked: return "Centimeters"
        return "Millimeters"
    def update_unit_labels(sender, e):
        try:
            ...
            print('Unit label update failed: %s' % ex)#<-replace
    ...
    def first_curve_figure(figs):
        ...
        for i in range(0, n):
            ...
            try:
                if hasattr(f, 'IsReference') and f.IsReference: continue
                t = f.GetType().Name.upper()
                if 'POINT' in t: continue
    ...
    # ---------- NEW: central close/cleanup helper ----------
    def close_form_safely():
        try:
            ...
            pass
        try:
            if hasattr(sel_sketch, 'my_filter') and sel_sketch.my_filter is not None:
                sel_sketch.my_filter.Reset()
        except:
            pass
        try:
            clear_selection_now_and_soon(root, host_form=form, delay_ms=10)
        except:
            pass
        try:
            form.Close()
    ...
    def ok_click(sender, e):
        ...
        try:
            if rb_mm.Checked:
                ...
                show_error('Please select a valid unit.', 'Invalid Units'); return
        except Exception as ex:
            show_error('Failed to set units: %s' % ex, include_trace=True); return
        Use3DPath = chk_3d.Checked
        # Profile type from radios
        if   rb_circle.Checked: ProfileTypeIndex = 0
        elif rb_rect.Checked:   ProfileTypeIndex = 1
        else:                   ProfileTypeIndex = -1
        try:
            ProfileSize = float(num_size.Value)
            if ProfileSize <= 0: show_error('Profile size must be > 0.', 'Invalid Size'); return
        except Exception as ex:
            show_error('Invalid profile size: %s' % ex); return
        try:
            IsHollow = chk_hollow.Checked
            Thickness = float(num_thick.Value)
            if IsHollow:
                if Thickness <= 0: show_error('Wall thickness must be > 0.', 'Invalid Thickness'); return
                if ProfileTypeIndex in (0,1) and Thickness >= (ProfileSize/2.0):
                    show_error('Wall thickness must be < half the profile size.', 'Invalid Thickness'); return
        except Exception as ex:
            show_error('Invalid thickness value: %s' % ex); return
        if sel_sketch.Items.Count == 0 or sel_sketch.PreviousSelection is None:
            show_info("Please select a path sketch", "Input Required"); return
        try:
            selected_item = sel_sketch.PreviousSelection.Item(0)
        except Exception as ex:
            show_error('Could not read selected sketch: %s' % ex, include_trace=True); return
        target = getattr(selected_item, 'Target', None)
        try:
            target_type = str(target.GetType().Name) if target is not None else ''
        except:
            target_type = str(getattr(target, 'Type', ''))
        sketch_name = None
        try: sketch_name = clean_name(getattr(target, 'Name', None))
        except: pass
        if sketch_name is None:
            try: sketch_name = clean_name(sel_sketch.Items[0])
            except: sketch_name = None
        if sketch_name is None:
            show_error('Could not resolve the selected sketch name.', 'Sketch Error'); return
        PathSketch = None
        try:
            if Use3DPath:
                try: PathSketch = MyPart.Get3DSketch(sketch_name)
                except Exception: PathSketch = MyPart.Get3DSketch(str(sel_sketch.Items[0]))
            else:
                try: PathSketch = MyPart.GetSketch(sketch_name)
                except Exception: PathSketch = MyPart.GetSketch(str(sel_sketch.Items[0]))
        except Exception as ex:
            show_error('Use 3D Path must match the selected sketch type. Failed getting "%s". Details: %s' %
                       (sketch_name, ex), '2D vs 3D Sketch Error', include_trace=True); return
        if PathSketch is None:
            show_error("A Path Sketch must be selected.", "Missing Path Sketch"); return
        try:
            is3d = '3D' in PathSketch.GetType().Name
            if is3d and not Use3DPath:
                show_error('Selected sketch is 3D. Check "Use 3D Path".', '3D Path Required'); return
            if (not is3d) and Use3DPath:
                show_error('Selected sketch is 2D. Uncheck "Use 3D Path" or pick a 3D sketch.', '2D Path Selected'); return
        except: pass
        FirstFigure = first_curve_figure(PathSketch.Figures)
        if FirstFigure is None:
            show_error("The path sketch must contain a valid non-reference curve.", "Invalid Path Geometry"); return
        try:
            def as_xyz(p):
                ...
                try:
                    n = len(p)
                    if n == 3: return [float(p[0]), float(p[1]), float(p[2])]
                    if n == 2: return [float(p[0]), float(p[1]), 0.0]
                except: pass
            ...
            def list_features(part):
                ...
                return feats
            def feat_key(f):
                try: h = int(f.GetHashCode())
                except: h = None
                try: n = str(f.Name)
                except: n = ""
                try: t = str(f.GetType().Name)
                except: t = ""
                return (h, n, t)
            if hasattr(FirstFigure, 'GetPointAt'):
                if is3d:
                    ...
                    sp2 = FirstFigure.GetPointAt(0.0)  # (u,v)
                ...
                try:
                    ...
                    for f in post_feats:
                        if feat_key(f) not in pre_keys:
                            try:
                                MyPart.RemoveFeature(f)  # precise object removal
                            except:
                                pass
                except:
                    pass
            ...
            clear_selection_now_and_soon(root, host_form=form, delay_ms=25)
            close_form_safely()
            return
        except Exception as ex:
    ...
    def cancel_click(sender, e):
        close_form_safely()
    btn_ok = create_professional_button("Advanced Sweep Tool", True); btn_ok.Dock = DockStyle.Fill
    add_control_row(btn_ok, extra_margin_bottom=8, fixed_height=60)
    btn_cancel = create_professional_button("Close", False); btn_cancel.Dock = DockStyle.Fill
    ...
    table.PerformLayout(); main_panel.PerformLayout()
    try:
        ...
        print('Form sizing warning: %s' % ex)#<-replace
    try:
        form.Show()
        # also clear right after showing / when re-activated
        try:
            clear_selection_now_and_soon(root, host_form=form, delay_ms=10)
            form.Activated += lambda *_: clear_selection_now_and_soon(root, host_form=form, delay_ms=10)
        except:
            pass
    except Exception as ex:
